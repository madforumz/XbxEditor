...
Public Class MidnightClub
    ...
    Public Function TitleIDVerify(ByVal filepath As String)
        ...
        BR.Close()
        If TitleID = "545407F8" Then ' Enter Ur TitleID Here ?
            readfile()
            Money.Enabled = True
            Save.Enabled = True
            MaxBB.Enabled = True
    ...
    Private Sub Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Save.Click
        ...
        Try
            Resign()
            savefile()
              MessageBoxEx.Show("Saved And Rehash And Resigned", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
        Catch ex As Exception
            MessageBoxEx.Show("Can't Save Your Midnight Club LA  Gamesave", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
            resetform()
        End Try
    End Sub
    Public Sub savefile()
        Dim writer As New PackageIO.Writer(filepath, Endian.Big)
        writer.Position = &H5DFC4
        writer.WriteInt32(Money.Value)
        writer.Position = &H5DFCC
        writer.WriteInt32(Rep.Value)
        writer.Close()
    End Sub
    Public Sub Resign()
